...
std::unordered_map<std::string, Shader*> Shader::caching; 
...
{
	std::string uniquepath = vertexShaderPath + "|" +  fragmentShaderPath;
	if (caching.find(uniquepath) != caching.end() ) 
	{
		return caching[uniquepath];
	}

	Shader* shader = new Shader();
	shader->LoadFromGLSLTextFiles(vertexShaderPath, fragmentShaderPath);
	return caching[uniquepath] = shader;
}
